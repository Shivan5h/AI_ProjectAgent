...
def load_project_files(project_path):
    """Load all code files from project directory"""
    code_files = []
...
def create_vectorstore(files):
    """Create ChromaDB vectorstore from project files"""
...
def groq_chat_completion(prompt, context=""):
    """Get response from Groq API with project context"""
    full_prompt = f"""
    Project Context:
    {context}
...
def get_relevant_context(query):
    """Retrieve relevant code context from vectorstore"""
...
# Streamlit UI
st.title("🧠 Agentic Project Explainer")
st.caption("Understand and modify any code project with AI assistance")
